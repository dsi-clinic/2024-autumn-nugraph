...
class NuGraphBlock(MessagePassing):  # pylint: disable=abstract-method
    ...
    def __init__(self, source_features: int, target_features: int, out_features: int):
        ...
        self.edge_net = nn.Sequential(
            nn.Linear(source_features + target_features, 1), nn.Sigmoid()
        )
...
class NuGraphCore(nn.Module):
    ...
    def forward(self, data: Data) -> None:
        ...
        if (
            "opflashsumpe" in data.node_types
            and ("sp", "connection", "opflashsumpe") in data.edge_types
        ):
            # message-passing from space points to PMTs
            data["opflashsumpe"].x = self.checkpoint(
                self.nexus_to_pmt,
                (data["sp"].x, data["opflashsumpe"].x),
