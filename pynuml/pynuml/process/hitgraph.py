...
class HitGraphProducer(ProcessorBase):
    ...
    def __init__(
        ...
        self.transform = pyg.transforms.Compose(
            (pyg.transforms.Delaunay(), pyg.transforms.FaceToEdge())
        )
    ...
    def __call__(self, evt: "pynuml.io.Event") -> tuple[str, Any]:
        ...
        if self.semantic_labeller:
            ...
            g4_id = g4_id.sort_values(
                by=[energy_col], ascending=False, kind="mergesort"
            ...
            if self.label_position:
                edeps = edeps[
                    ["hit_id", "energy", "x_position", "y_position", "z_position"]
                ]
        ...
        # node true position
        if self.label_position:
            data["hit"].c = torch.tensor(
                hits[["x_position", "y_position", "z_position"]].values
            ).float()
        ...
        # 2D graph edges
        data["hit", "delaunay", "hit"].edge_index = self.transform(
            data["hit"]
        ).edge_index
        edge_plane = []
        ...
        if self.semantic_labeller:
            ...
            if self.store_detailed_truth:
                ...
                data["hit"].parent_id = torch.tensor(
                    hits["parent_id"].fillna(-1).values
                ).long()
        ...
        if self.optical:
            ...
            data["opflash"].pos = torch.tensor(
                opflash[["wire_pos_0", "wire_pos_1", "wire_pos_2"]].values
            ).float()

            # node features
            data["ophits"].x = torch.tensor(
                ophits[
                    [
                        "amplitude",
                        "area",
            ...
            data["opflashsumpe"].x = torch.tensor(
                sum_pe[["pmt_channel", "sumpe"]].values
            ).float()
